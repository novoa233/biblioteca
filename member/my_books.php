				...
				for($i=0; $i<$rows; $i++)
					if(isset($_POST['cb_book'.$i]))
					{
						...
						if($days > 0)
						{
							...
							$query->execute();
							echo '<script>
									document.getElementById("error").innerHTML += "Una multa de $ '.$penalty.' fue cursada por no entregar tu libro '.$_POST['cb_book'.$i].' por '.$days.' días después del tiempo convenido de entrega.<br />";
